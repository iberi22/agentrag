...
Cortex needs an **Evolve Module** — a self-contained, autonomous agentic subsystem that continuously improves the entire Cortex memory architecture by running automated research experiments, synthesizing SOTA findings, and applying verified improvements. This issue tracks the full design, implementation, and integration plan.
...
- [ ] **Autonomous self-improvement** — The Evolve agent runs independently, testing hypotheses against Cortex's own memory subsystem
- [ ] **Cortex-as-MCP integration** — Use Cortex's own memory (running in Docker) as the MCP backend for recording evolution state, experiment logs, and knowledge synthesis
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│                    CORTEX EVOLVE MODULE (Autonomous Loop)                    │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
...
│                            (Apply / Discard)                                 │
│                                                                              │
│  State persisted in: SurrealDB via Cortex MCP (localhost:8003)              │
...
| **🧪 Experimenter** | Modifies Cortex code (memory module) to implement hypotheses | autoresearch `train.py` modification |
...
### Phase 1: Cortex MCP Integration (Skill + Configuration)

- [ ] Create `.agents/skills/cortex-memory/SKILL.md` — Skill document for using Cortex memory as MCP
- [ ] Configure Cortex Docker MCP endpoint (`localhost:8003`) for agent state persistence
- [ ] Define MCP tools: `evolve_store_experiment`, `evolve_get_history`, `evolve_store_hypothesis`, `evolve_get_metrics`
- [ ] Verify Cortex health endpoint responds at `http://localhost:8003/health`
...
- [ ] `evaluator.rs` — Runs LoCoMo and Evo-Memory benchmarks against current Cortex instance
...
- [ ] Historical experiment tracking in SurrealDB via Cortex MCP
...
- [ ] Establish baseline scores on both benchmarks with current Cortex implementation
...
- [ ] Deploy Evolve agent as background Tokio task within Cortex runtime
