...
impl AgentMessage {
    ...
    pub fn heartbeat(sender: &str) -> Self {
        Self::new(sender, MessageType::Heartbeat, serde_json::json!({ "status": "alive" }))
    }
...
impl MessageBus {
    ...
    pub async fn register_agent(&self, agent_id: &str, name: &str, capabilities: Vec<String>) -> Result<mpsc::Receiver<AgentMessage>, MessageBusError> {
        let (tx, rx) = mpsc::channel(100);
        
        {
            let mut queues = self.queues.write().await;
            if queues.contains_key(agent_id) {
                return Err(MessageBusError::AgentAlreadyRegistered(agent_id.to_string()));
            }
        ...
        
        {
            ...
            agents.insert(agent_id.to_string(), AgentInfo {
    ...
    pub async fn subscribe(&self, agent_id: &str, topic: &str) -> Result<(), MessageBusError> {
        ...
        let mut topics = self.topics.write().await;
        topics.entry(topic.to_string()).or_default().insert(agent_id.to_string());
        
        {
            let mut metrics = self.metrics.write().await;
            metrics.topic_subscriptions = topics
                .iter()
    ...
    pub async fn publish(&self, message: AgentMessage) -> Result<usize, MessageBusError> {
        ...
        tracing::debug!("Message {} published to {} receivers", message.id, sent_count);
    ...
    pub async fn receive(&self, agent_id: &str, timeout: Option<u64>) -> Result<Option<AgentMessage>, MessageBusError> {
        let queues = self.queues.read().await;
        
        if let Some(tx) = queues.get(agent_id) {
            drop(queues);
            
            let msg = if let Some secs = timeout {
                tokio::time::timeout(
    ...
    pub async fn send_to_dlq(&self, message: AgentMessage, reason: &str) -> Result<(), MessageBusError> {
    ...
    pub async fn send_to_dlq_with_retry(&self, message: AgentMessage, reason: &str, retry_count: u32) -> Result<(), MessageBusError> {
        ...
        tracing::error!("Message sent to DLQ after {} retries: {}", retry_count, reason);
...
mod tests {
    ...
    async fn test_register_unregister() {
        ...
        // Register agent
        let rx = bus.register_agent("agent1", "Test Agent", vec!["test".to_string()]).await;
        assert!(rx.is_ok());
    ...
    async fn test_publish_subscribe() {
        ...
        // Register agents
        bus.register_agent("agent1", "Agent 1", vec![]).await.unwrap();
        bus.register_agent("agent2", "Agent 2", vec![]).await.unwrap();
        
        // Subscribe agent1 to topic
        bus.subscribe("agent1", "news").await.unwrap();
        
        // Publish to topic
        let msg = AgentMessage::new("agent2", MessageType::Task, serde_json::json!({"text": "hello"}))
            .on_topic("news");
    ...
    async fn test_direct_message() {
        ...
        bus.register_agent("sender", "Sender", vec![]).await.unwrap();
        bus.register_agent("receiver", "Receiver", vec![]).await.unwrap();
        
        let id = bus.send_direct(
            "sender",
            "receiver",
    ...
    async fn test_broadcast() {
        ...
        bus.register_agent("agent1", "Agent 1", vec![]).await.unwrap();
        bus.register_agent("agent2", "Agent 2", vec![]).await.unwrap();
        bus.register_agent("agent3", "Agent 3", vec![]).await.unwrap();
        
        let id = bus.broadcast(
            "sender",
            serde_json::json!({"text": "broadcast"}),
            None
        ).await.unwrap();
    ...
    async fn test_request_response() {
        ...
        bus.register_agent("client", "Client", vec![]).await.unwrap();
        bus.register_agent("server", "Server", vec![]).await.unwrap();
    ...
    async fn test_get_agent() {
        ...
        bus.register_agent("agent1", "Test Agent", vec!["capability1".to_string()]).await.unwrap();
    ...
    async fn test_dlq_operations() {
        ...
        bus.send_to_dlq(msg.clone()).await.unwrap();
