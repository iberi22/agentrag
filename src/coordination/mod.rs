...
impl Coordinator {
    ...
    pub async fn broadcast(&self, event: Event) {
        ...
        for agent in agents {
            self.send_to("coordinator".to_string(), agent, event.clone()).await;
        }
    }
...
impl AgentRegistry {
    ...
    pub async fn register(
        ...
        let rx = self.bus.register_agent(id, name, capabilities.clone()).await?;
        ...
        // Store metadata
        {
            ...
            metadata.insert(id.to_string(), AgentMetadata {
                id: id.to_string(),
                name: name.to_string(),
                role,
                description,
    ...
    pub async fn get_receiver(&self, id: &str) -> Option<tokio::sync::mpsc::Receiver<AgentMessage>> {
...
impl CoordinationService {
    ...
    pub async fn dispatch_task(&self, task_id: &str) -> Result<TaskDistribution, MessageBusError> {
        ...
        let agents = self.registry.find_by_capability(
            &task.required_capabilities.first().unwrap_or(&"default".to_string())
        ).await;
        
        if agents.is_empty() {
            return Err(MessageBusError::SendFailed("No agents available".to_string()));
        }
    ...
    pub async fn complete_task(&self, task_id: &str, result: serde_json::Value) -> Result<(), MessageBusError> {
        let mut tasks = self.tasks.write().await;
    ...
    pub async fn metrics(&self) -> CoordinationMetrics {
        ...
        CoordinationMetrics {
            ...
            active_agents: agents.iter().filter(|a| a.status == AgentStatus::Active).count(),
            total_tasks: tasks.len(),
            pending_tasks: tasks.iter().filter(|t| t.status == CoordinationTaskStatus::Pending).count(),
            completed_tasks: tasks.iter().filter(|t| t.status == CoordinationTaskStatus::Completed).count(),
            failed_tasks: tasks.iter().filter(|t| t.status == CoordinationTaskStatus::Failed).count(),
