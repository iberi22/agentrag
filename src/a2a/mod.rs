//! Agent-to-Agent (A2A) protocol primitives used by Cortex.
//!
//! The module provides:
...
pub trait TaskHandler: Send + Sync {
    ...
    async fn get_task(&self, task_id: &str) -> Result<types::Task, String>;

    /// Requests task cancellation.
    async fn cancel_task(&self, task_id: &str) -> Result<(), String>;
}

/// JSON-RPC dispatcher for the A2A task methods supported by Cortex.
...
/// Builds Cortex's default A2A discovery card.
pub fn create_cortex_agent_card() -> types::AgentCard {
    types::AgentCard {
...
mod tests {
    ...
    fn test_message() -> types::Message {
        types::Message {
            role: "user".to_string(),
            content: types::MessageContent::Text("hello cortex".to_string()),
        }
    }

    fn test_server() -> A2AServer {
        A2AServer::new(create_cortex_agent_card(), Arc::new(TestTaskHandler::new()))
    }

    #[test]
    fn create_cortex_agent_card_exposes_expected_metadata() {
        let card = create_cortex_agent_card();

        assert_eq!(card.name, "Cortex");
    ...
    fn client_registry_returns_registered_agent() {
        ...
        let card = create_cortex_agent_card();
        client.register_agent("cortex", card.clone());

        assert_eq!(client.get_registered_agent("cortex"), Some(card));
