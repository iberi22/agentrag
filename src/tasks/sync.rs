...
//! Tasks are stored in Cortex, Planka is just a view/sync target
...
/// Sync status between Cortex and Planka
#[derive(Debug, Clone)]
pub enum SyncStatus {
    Synced,
    PendingUpload,
...
impl<S: TaskStore> PlankaSyncService<S> {
    ...
    /// Import tasks from Planka to Cortex
...
mod tests {
    ...
    async fn test_task_creation() {
        let (store, service, _sync) = create_task_system();

        let task = service
            .create_task("Test task", "Cortex", "test-user")
            .await
            .unwrap();

        assert_eq!(task.title, "Test task");
        assert_eq!(task.project, "Cortex");
    ...
    async fn test_task_board() {
        ...
        service
            .create_task("Task 1", "Cortex", "user")
            .await
            .unwrap();

        let task2 = service
            .create_task("Task 2", "Cortex", "user")
            .await
            .unwrap();
        service
        ...
        let board = service.get_task_board("Cortex").await.unwrap();

        assert!(board.get("Backlog").unwrap().len() >= 1);
        assert!(board.get("In Progress").unwrap().len() >= 1);
    }
}
